...
public class BuildMenuUI : MonoBehaviour
{
    ...
    [Tooltip("Предпочитать каталог из БД/JSON вместо ручного списка ниже, чтобы не дублировать данные.")]
    [SerializeField] private bool preferCatalogOverManualList = true;

    [Header("Legacy: список из инспектора (если каталог пуст)")]
    ...
    [SerializeField] private PCComponentModelViewer modelViewer;
    ...
    private void Start()
    {
        ...
        {
            buildMenuController = FindFirstObjectByType<BuildMenuController>();
        }
    ...
    private string GetCategoryFromToggleName(string toggleName)
    {
        ...
        switch (n)
        {
            ...
            case "cooler":
                return "CPU_COOLER";
            default:
                Debug.LogWarning($"BuildMenuUI: неизвестное имя вкладки '{toggleName}', используйте латиницу или см. GetCategoryFromToggleName.");
    ...
    public void RefreshCards()
    {
        ...
        if (UseCatalog())
        {
            ...
            if (filtered.Count == 0)
            {
                Debug.LogWarning("BuildMenuUI: каталог пуст или не удалось загрузить компоненты из базы. Проверьте подключение и настройки.");
            }
    ...
    private void CreateCardFromCatalog(PcComponentData data)
    {
        ...
        if (prefab == null)
        {
            Debug.LogWarning($"BuildMenuUI: для категории '{data.CategoryCode}' не найден префаб в PcPrefabCatalogMap (запись id={data.DatabaseId}).");
            return;
        }
        ...
        if (buttonTransform != null)
        {
            ...
            if (useBtn != null)
            {
                ...
                useBtn.onClick.AddListener(() =>
                {
                    ...
                    GameObject modelPrefab = prefabCatalogMap.GetVisualModelForTier(data.CategoryCode, data.ModelTier);
                    if (modelPrefab == null && prefab != null)
                    {
                        modelPrefab = prefab.gameObject;
                    }

                    if (modelPrefab != null)
                    {
                        CloseBuildMenuForModelViewer();
                        modelViewer.OpenViewer(modelPrefab);
                    }
                });
            }
        }
        ...
        if (buttonTransform != null)
        {
            ...
            if (useBtn != null)
            {
                ...
                useBtn.onClick.AddListener(() =>
                {
                    ...
                    GameObject modelPrefab = partData.PartPrefab != null ? partData.PartPrefab.gameObject : null;
                    if (modelPrefab != null)
                    {
                        CloseBuildMenuForModelViewer();
                        modelViewer.OpenViewer(modelPrefab);
    ...
    private void CloseBuildMenuForModelViewer()
    {
        if (buildMenuController != null)
        {
